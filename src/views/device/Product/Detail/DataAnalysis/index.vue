...
const save = async () => {
    ...
    if (res.status === 200) {
        onlyMessage(t('Detail.DataAnalysis.index.5rcy2jz4zyg0'));
        getProductCode();
    }
};
...
const debug = () => {
    if (productStore.current.transportProtocol === 'MQTT') {
        if (topic.value !== '') {
            ...
            onlyMessage(t('Detail.DataAnalysis.index.5rcy2jz506g0'), 'error');
        }
    } else {
        if (url.value !== '') {
            test({
            ...
            onlyMessage(t('Detail.DataAnalysis.index.5rcy2jz50e00'), 'error');
        }
    }
};

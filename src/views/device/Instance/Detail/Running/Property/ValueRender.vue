...
const getDetail = (_type: string) => {
    ...
    if (_type === 'img') {
        if (isHttps && value?.formatValue.indexOf('http:') !== -1) {
            onlyMessage(t('Running.Property.ValueRender.5rcyqc5r2yw0'), 'error');
        } else if (temp.value) {
            onlyMessage(t('Running.Property.ValueRender.5rcyqc5r5940'), 'error');
        } else {
            flag =
                ['.jpg', '.png'].find((item) =>
                    value?.formatValue.includes(item),
        ...
        if (isHttps && value?.formatValue.indexOf('http:') !== -1) {
            ...
            onlyMessage(t('Running.Property.ValueRender.5rcyqc5r5kg0'), 'error');
        } else {
            flag =

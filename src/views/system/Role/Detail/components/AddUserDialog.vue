<template>
    <j-modal visible :title="$t('Detail.components.AddUserDialog.5rkjopn8q0s0')" width="1000px" @ok="confirm" @cancel="emits('update:visible', false)">
        <!-- <j-advanced-search
            :columns="columns"
            type="simple"
...
const confirm = () => {
    if (selectedRowKeys.value.length < 1) {
        onlyMessage(t('Detail.components.AddUserDialog.5rkjopn8ru80'), 'error');
    } else {
        bindUser_api(props.roleId, selectedRowKeys.value).then((resp) => {
            if (resp.status === 200) {
                onlyMessage(t('Detail.components.AddUserDialog.5rkjopn8s180'));
                emits('refresh');
